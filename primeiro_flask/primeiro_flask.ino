...
long tempo, deltaTempo, minutos=0, segundos=0, centesimos=0;   
int apresentacao = 0;

//Funcao para atualizar o flask
void atualizarTempoNoServidor(long minutos, long segundos, long centesimos) {
    HTTPClient http;
    String url = "http://192.168.4.2:5000/atualizar_cronometro";
    
    http.begin(url);
    http.addHeader("Content-Type", "application/json");

    // Formata os dados em JSON
    String jsonData = "{\"minutos\":" + String(minutos) + 
                      ", \"segundos\":" + String(segundos) + 
                      ", \"centesimos\":" + String(centesimos) + "}";
...
void loop() {
  ...
  if(!cavaloJaPassou && !pareiCronometro){
    ...
    display.display();
    atualizarTempoNoServidor(0, 0, 0);
  }
  ...
  if(cavaloJaPassou){
    ...
    atualizarTempoNoServidor(minutos, segundos, centesimos);
  }
}